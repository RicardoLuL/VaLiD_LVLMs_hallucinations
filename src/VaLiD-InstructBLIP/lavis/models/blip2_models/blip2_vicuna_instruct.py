...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def generate(
        ...
        if image.dim() == 5:
            ...
            with self.maybe_autocast():
                image_embeds = self.ln_vision(self.visual_encoder(image, layer_index))       # Here encode image
            image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
        ...
        with self.maybe_autocast():
            ...
            outputs = self.llm_model.generate(
                ...
                use_valid=use_valid, 
                valid_beta=valid_beta if valid_beta is not None else None,
                valid_alpha=valid_alpha if valid_alpha is not None else None
            )
