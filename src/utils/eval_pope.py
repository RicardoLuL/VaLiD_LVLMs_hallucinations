def eval_pope(data, file_dir):
    for cate in ['random', 'popular', 'adversarial']:
        eval_data = [item for item in data if item['cate'] == cate]
        acc, precision, recall, f1, yes, unknown = cal_results_of_pope(eval_data)
        with open(f'{file_dir}.txt', 'a') as file:
            file.write(f'{cate}: \nacc:{acc}, \nprecision:{precision}, \nrecall:{recall}, \nf1:{f1}, \nyes:{yes}, \nunknown:{unknown}')
            file.write("\n"+"="*75+"\n")
            file.close()

def cal_results_of_pope(data):
